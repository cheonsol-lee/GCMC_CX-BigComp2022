...
from model_regression import BiDecoder, GCMCLayer
...
def evaluate(args, net, dataset, segment='valid'):
    possible_rating_values = [i*0.5 for i in range(1,11)]
    ...
    true_labels = rating_values.reshape(-1, 1).type(th.FloatTensor).to(args.device)
    rmse = ((pred_ratings - true_labels) ** 2.).mean().item()
    rmse = np.sqrt(rmse)
    return rmse, pred_ratings
...
#     print(args)
    ...
    nd_possible_rating_values = th.FloatTensor([i*0.5 for i in range(1,11)]).to(args.device)
    rating_loss_net = nn.MSELoss().cuda()
...
#     print("Loading network finished ...\n")
    ...
    train_loss_logger = MetricLogger(['iter', 'loss', 'rmse'], ['%d', '%.4f', '%.4f'],
                                     f"{args.save_dir}/train_loss_{args.save_id}.csv")
    valid_loss_logger = MetricLogger(['iter', 'rmse'], ['%d', '%.4f'],
                                     f"{args.save_dir}/valid_loss_{args.save_id}.csv")
    test_loss_logger = MetricLogger(['iter', 'rmse'], ['%d', '%.4f'],
                                    f"{args.save_dir}/test_loss_{args.save_id}.csv")
...
#     print("Start training ...")
    ...
    for iter_idx in range(1, args.train_max_iter):
        ...
        pred_ratings = net(dataset.train_enc_graph, dataset.train_dec_graph,
                           dataset.user_feature, dataset.movie_feature)
        true_labels = train_gt_ratings.reshape(-1, 1).type(th.FloatTensor).to(args.device)
        loss = rating_loss_net(input=pred_ratings ,target=true_labels)
...
#             print(torch_net_info(net, save_path=f"{args.save_dir}/net_{args.save_id}.txt"))
            torch_net_info(net, save_path=f"{args.save_dir}/net_{args.save_id}.txt")

        # print(nd_possible_rating_values.view(1, -1).shape)

        rmse = ((pred_ratings - true_labels) ** 2).sum()
        ...
        if iter_idx % args.train_log_interval == 0:
            train_loss_logger.log(iter=iter_idx,
                                  loss=count_loss/(iter_idx+1), rmse=rmse)
        ...
        if iter_idx % args.train_valid_interval == 0:
            ...
            if valid_rmse < best_valid_rmse:
                ...
                # GPU사용할 때, 예외처리
                if args.device==0:
                    best_pred_result['test_pred'] = test_pred.cpu().reshape(-1)
                else:
...
#         if iter_idx  % args.train_log_interval == 0:
#             print(logging_str) # Log print
            
        # break #############
    print('Best Iter Idx={}, Best Valid RMSE={:.4f}, Best Test RMSE={:.4f}'.format(
        best_iter, best_valid_rmse, best_test_rmse))

    test_result_df = pd.DataFrame(best_pred_result)
    test_result_df.to_csv(f"{args.save_dir}/{args.save_id}_test_best.csv")
